...
const werewolf_1 = require("../../services/werewolf");
const mockRedis = {
    setex: vitest_1.vi.fn(),
    get: vitest_1.vi.fn(),
    del: vitest_1.vi.fn(),
};
(0, vitest_1.describe)('WerewolfService', () => {
    let service;
    ...
    (0, vitest_1.beforeEach)(() => {
        ...
        service = new werewolf_1.WerewolfService(mockRedis);
    });
    (0, vitest_1.describe)('createGame', () => {
        (0, vitest_1.it)('应该创建新游戏', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            (0, vitest_1.expect)(game).toBeDefined();
            (0, vitest_1.expect)(game.id).toMatch(/^ww_/);
            (0, vitest_1.expect)(game.roomId).toBe('room-123');
            (0, vitest_1.expect)(game.phase).toBe('waiting');
            (0, vitest_1.expect)(game.players.size).toBe(0);
        });
        (0, vitest_1.it)('应该使用自定义配置', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const customConfig = {
                minPlayers: 6,
                maxPlayers: 6,
            ...
            (0, vitest_1.expect)(game.config.maxPlayers).toBe(6);
            (0, vitest_1.expect)(game.config.roles.werewolf).toBe(2);
        });
    });
    (0, vitest_1.describe)('joinGame', () => {
        (0, vitest_1.it)('应该允许 AI 加入等待中的游戏', async () => {
            const mockGame = {
                id: 'ww_test',
                roomId: 'room-123',
                config: {
            ...
            (0, vitest_1.expect)(result.player).toBeDefined();
            (0, vitest_1.expect)(result.player?.agentId).toBe('ai-1');
        });
        (0, vitest_1.it)('应该拒绝加入已满的游戏', async () => {
        ...
        (0, vitest_1.it)('应该拒绝加入已开始的游戏', async () => {
            const mockGame = {
                id: 'ww_test',
                roomId: 'room-123',
                config: { maxPlayers: 12 },
            ...
            const result = await service.joinGame('ww_test', agent);
            (0, vitest_1.expect)(result.success).toBe(false);
            (0, vitest_1.expect)(result.error).toBe('Game already started');
        });
    });
    (0, vitest_1.describe)('角色分配', () => {
        (0, vitest_1.it)('应该正确分配角色', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            // 模拟 12 个 AI 加入
            for (let i = 0; i < 12; i++) {
            ...
            (0, vitest_1.expect)(werewolves).toBe(4);
            (0, vitest_1.expect)(villagers).toBe(4);
            (0, vitest_1.expect)(seers).toBe(1);
        });
    });
    (0, vitest_1.describe)('游戏流程', () => {
        (0, vitest_1.it)('应该正确处理夜晚击杀', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            // 创建玩家
            for (let i = 0; i < 12; i++) {
            ...
            if (werewolf && target) {
                ...
                // 检查目标是否死亡
                (0, vitest_1.expect)(target.isAlive).toBe(false);
            }
        });
        (0, vitest_1.it)('应该正确处理女巫救人', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            for (let i = 0; i < 12; i++) {
                const agent = createMockAgent(`ai-${i}`, `Agent-${i}`);
            ...
            const target = Array.from(game.players.values()).find(p => p.role !== 'werewolf' && p.role !== 'witch');
            if (werewolf && witch && target) {
                ...
                await service.endNight(game.id);
                // 目标应该被救活
                (0, vitest_1.expect)(target.isAlive).toBe(true);
                (0, vitest_1.expect)(witch.abilities.witchHeal).toBe(false);
            }
        });
    });
    (0, vitest_1.describe)('游戏结束判断', () => {
        (0, vitest_1.it)('应该正确判断狼人获胜', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            for (let i = 0; i < 12; i++) {
                const agent = createMockAgent(`ai-${i}`, `Agent-${i}`);
            ...
            (0, vitest_1.expect)(ended).toBe(true);
            (0, vitest_1.expect)(game.winner).toBe('werewolf');
            (0, vitest_1.expect)(game.phase).toBe('ended');
        });
        (0, vitest_1.it)('应该正确判断好人获胜', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            for (let i = 0; i < 12; i++) {
                const agent = createMockAgent(`ai-${i}`, `Agent-${i}`);
            ...
            const ended = await service.checkGameEnd(game.id);
            (0, vitest_1.expect)(ended).toBe(true);
            (0, vitest_1.expect)(game.winner).toBe('villager');
            (0, vitest_1.expect)(game.phase).toBe('ended');
        });
    });
    (0, vitest_1.describe)('AI 策略', () => {
        (0, vitest_1.it)('狼人应该优先刀神职', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            for (let i = 0; i < 12; i++) {
                const agent = createMockAgent(`ai-${i}`, `Agent-${i}`);
            ...
            if (werewolf) {
                ...
                // 优先刀神职或高威胁目标
                (0, vitest_1.expect)(['seer', 'witch', 'hunter', 'guard', 'villager']).toContain(target?.role);
            }
        });
        (0, vitest_1.it)('预言家应该查验可疑目标', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            for (let i = 0; i < 12; i++) {
                const agent = createMockAgent(`ai-${i}`, `Agent-${i}`);
            ...
            if (seer) {
                ...
                (0, vitest_1.expect)(action).toBeDefined();
                (0, vitest_1.expect)(action.target).toBeDefined();
                (0, vitest_1.expect)(action.result).toMatch(/werewolf|good/);
            }
        });
    });
    (0, vitest_1.describe)('性能', () => {
        (0, vitest_1.it)('应该在 100ms 内处理 12 个 AI 的夜间行动', async () => {
            mockRedis.setex.mockResolvedValue('OK');
            const game = await service.createGame('room-123');
            for (let i = 0; i < 12; i++) {
                const agent = createMockAgent(`ai-${i}`, `Agent-${i}`);
