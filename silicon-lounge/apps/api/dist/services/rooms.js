...
class RoomService {
    ...
    async getRooms(roomIds) {
        ...
        for (let i = 0; i < roomIds.length; i++) {
            ...
            if (data) {
                rooms.set(roomIds[i], JSON.parse(data));
            }
        }
        return rooms;
    }
    ...
    `;
        const result = await this.redis.eval(luaScript, 2, `room:${roomId}`, `room:${roomId}:agents`, agent.id, JSON.stringify({ ...agent, name: (0, shared_1.generateAnonymousName)() }), metadata.maxAgents.toString());
        if (result[0] !== 0) {
     ...
     * 批量加入房间
     */
    async batchJoinRoom(roomId, agents) {
        const metadata = await this.getRoomMetadata(roomId);
        if (!metadata) {
    ...
    async addMessage(roomId, message) {
        const pipeline = this.redis.pipeline();
        // 添加到 Stream
        pipeline.xadd(`room:${roomId}:stream`, '*', // 自动生成 ID
     ...
     * 批量添加消息
     */
    async batchAddMessages(roomId, messages) {
        const pipeline = this.redis.pipeline();
        for (const message of messages) {
            pipeline.xadd(`room:${roomId}:stream`, '*', 'data', JSON.stringify(message));
        }
        pipeline.xtrim(`room:${roomId}:stream`, 'MAXLEN', '~', this.MESSAGE_RETENTION);
     ...
     * 获取最近消息
     */
    async getRecentMessages(roomId, count = 100) {
        const results = await this.redis.xrevrange(`room:${roomId}:stream`, '+', // 最新
        '-', // 最旧
        'COUNT', count);
        return results
            .reverse() // 按时间正序
     ...
     * 获取消息范围（用于分页）
     */
    async getMessagesRange(roomId, startId, endId, count = 100) {
        const results = await this.redis.xrange(`room:${roomId}:stream`, startId, endId, 'COUNT', count);
     ...
     * 更新代理 socket
     */
    async updateAgentSocket(roomId, agentId, socketId) {
