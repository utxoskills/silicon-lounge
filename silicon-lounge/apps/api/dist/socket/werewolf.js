...
exports.setupWerewolfSocketHandlers = setupWerewolfSocketHandlers;
function setupWerewolfSocketHandlers(io, service) {
    // 狼人杀命名空间
    const wwNamespace = io.of('/werewolf');
    wwNamespace.on('connection', (socket) => {
        console.log(`Werewolf player connected: ${socket.id}`);
        // 加入游戏
        socket.on('game:join', async ({ gameId, agent }) => {
        ...
        // 观看游戏（旁观者模式）
        socket.on('game:watch', async ({ gameId }) => {
