...
export class QuizService {
  private redis: Redis;
  private readonly GAME_TTL = 3600; // 1 小时
   ...
   * 生成题目
   */
  private generateQuestions(difficulty: QuizDifficulty, count: number): QuizQuestion[] {
    ...
    for (const [type, templates] of Object.entries(QUESTION_TEMPLATES)) {
      for (const template of templates) {
        allQuestions.push({
          id: `q_${uuidv4()}`,
          type: (template.type as any) || 'open',
   ...
   * 获取最大玩家数
   */
  private getMaxPlayers(mode: QuizMode): number {
    switch (mode) {
      case '1v1': return 2;
   ...
   * 检查并自动开始
   */
  private async checkAndStart(gameId: string): Promise<void> {
    const game = await this.getGame(gameId);
    if (!game) return;
  ...
  ): Promise<{ success: boolean; error?: string; submittedAt?: number }> {
    const game = await this.getGame(gameId);
    if (!game) {
      return { success: false, error: 'Game not found' };
    }
   ...
   * 开始裁判评分
   */
  async startJudging(gameId: string): Promise<void> {
    const game = await this.getGame(gameId);
    if (!game) return;
  ...
  async submitJudgment(
    ...
    const judgmentRecord: QuizJudgment = {
    ...
    // 检查是否所有裁判都已提交
    const allSubmitted = game.referees.every(r => 
      r.judgments.filter(j => j.questionId === judgment.questionId).length >= game.players.length
    );
   ...
   * 结束评分
   */
  async endJudging(gameId: string): Promise<void> {
    const game = await this.getGame(gameId);
    if (!game) return;
    ...
    for (const player of game.players) {
      const judgments = game.referees.flatMap(r => 
        r.judgments.filter(j => j.playerId === player.agentId && j.questionId === game.questions[game.currentQuestion].id)
      );
   ...
   * 结束题目
   */
  async endQuestion(gameId: string, questionIndex: number): Promise<void> {
    const game = await this.getGame(gameId);
  ...
  async endGame(gameId: string): Promise<{
    ...
    playerRatings: { agentId: string; change: number }[];
    refereeRatings: { agentId: string; change: number }[];
  }> {
    const game = await this.getGame(gameId);
  ...
  calculateRefereeAccuracy(referee: QuizReferee, game: QuizGame): number {
    if (referee.judgments.length === 0) return 0;
    ...
    for (const judgment of referee.judgments) {
      ...
      const otherJudgments = game.referees
        ...
        .flatMap(r => r.judgments.filter(j => 
          j.questionId === judgment.questionId && j.playerId === judgment.playerId
        ));
  ...
  private calculateGameRatings(
    game: QuizGame,
    type: 'player' | 'referee'
  ): { agentId: string; change: number }[] {
    const ratings: { agentId: string; change: number }[] = [];

    if (type === 'player') {
      // 选手积分
      const sortedPlayers = game.players.sort((a, b) => 
        (game.scores.get(b.agentId) || 0) - (game.scores.get(a.agentId) || 0)
      );
   ...
   * 保存游戏
   */
  private async saveGame(game: QuizGame): Promise<void> {
