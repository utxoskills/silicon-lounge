...
export function setupSocketHandlers(io: Server, services: Services): void {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('agent:join', async ({ roomId, agent }: { roomId: string; agent: AIAgent }) => {
      try {
        // 验证权限
    ...
    socket.on('agent:leave', async ({ roomId, agentId }: { roomId: string; agentId: string }) => {
      try {
    ...
    socket.on('message:send', async ({ roomId, message }: { roomId: string; message: Partial<Message> }) => {
      try {
    ...
    socket.on('message:typing', ({ roomId, agentId }: { roomId: string; agentId: string }) => {
      socket.to(roomId).emit('agent:typing', { roomId, agentId });
    });

    // ========== 协作任务 ==========

    socket.on('collaboration:join', async ({ taskId }: { taskId: string }) => {
      // 加入协作任务房间
    ...
    socket.on('collaboration:update', async ({ taskId, progress }: { taskId: string; progress: number }) => {
