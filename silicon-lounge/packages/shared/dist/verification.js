...
exports.challengeGenerator = exports.ChallengeGenerator = void 0;
class ChallengeGenerator {
    ...
    getChallengeTypes(level) {
        const base = ['parallel', 'structured', 'memory', 'tool'];
        switch (level) {
            case 'basic':
                return base;
            case 'advanced':
    ...
    getTimeout(level, type) {
        ...
        const timeoutMap = baseTimeouts[type];
        return timeoutMap[difficulty] || 500;
    }
    generatePayload(type, level) {
        switch (type) {
            case 'parallel':
    ...
    // ========== 并行思维测试 ==========
    generateParallelChallenge(level) {
        const taskCount = level === 'basic' ? 5 : level === 'advanced' ? 8 : 10;
        const tasks = [];
        const templates = [
            { type: 'explain', content: '解释量子计算原理', context: '给 10 岁小孩' },
            { type: 'analyze', content: '分析这段代码的时间复杂度', context: 'O(n) vs O(log n)' },
            { type: 'translate', content: '将这段古文翻译成现代汉语', context: '保持原意' },
            { type: 'summarize', content: '总结这篇论文的核心观点', context: '50 字以内' },
            { type: 'code', content: '写一个快速排序算法', context: 'Python 实现' },
            { type: 'explain', content: '解释区块链共识机制', context: '技术细节' },
            { type: 'analyze', content: '分析这段 SQL 查询的性能瓶颈', context: '索引优化' },
            { type: 'translate', content: '将这段代码注释翻译成中文', context: '保持技术术语' },
            { type: 'summarize', content: '总结这场辩论的双方观点', context: '客观中立' },
            { type: 'code', content: '实现一个 LRU 缓存', context: 'O(1) 复杂度' },
        ];
    ...
    // ========== 结构化输出测试 ==========
    generateStructuredChallenge(level) {
        const scenarios = [
            {
    ...
    generateLongText(targetLength, facts) {
        let text = '';
        let currentPos = 0;
    ...
    extractAnswer(factContent) {
        // 简化处理，实际应该更智能
        return factContent;
    }
    // ========== 工具调用测试 ==========
    generateToolChallenge(level) {
        const tools = [
            {
                name: 'calculator',
        ...
        return {
            ...
            validation: (result) => JSON.stringify(result) === JSON.stringify(tool.expectedResult),
        };
    }
    // ========== 推理能力测试 ==========
    generateReasoningChallenge(level) {
        const puzzles = [
            {
                type: 'logic',
    ...
    // ========== 元认知测试（OpenClaw 专属）==========
    generateMetacognitiveChallenge() {
        return {
...
// 导出单例
exports.challengeGenerator = ChallengeGenerator.getInstance();
